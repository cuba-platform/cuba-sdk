...
@Parameters(commandDescription = "Upload component to SDK target repository")
class PushCommand : BasePushCommand() {

    @Parameter(
        names = ["--c", "--components"],
        description = "List of components via ',' in <name>:<version> or in full coordinates format <group>:<name>:<version>. Example: framework-cuba:7.2.1,addon-dashboard:3.2.1",
        hidden = true
    )
        ...
        checkRepositories(repositoryNames)?.let { repositories ->
            ...
            components.forEach {
                if (force(it) || !componentManager.isAlreadyInstalled(it)) {
                    ...
                    if (component != null) {
                        if (!componentsToResolve.contains(component)) {
                            componentsToResolve.add(component)
                        }
                    }
                }
            }
